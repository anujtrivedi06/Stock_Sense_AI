...
with st.sidebar:
    ...
    sl_mode = st.selectbox(
        ...
        format_func=lambda x: {"fixed": "Fixed %", "trailing": "Trailing %", "atr": "ATR-Based"}[x],
...
def display_stock_prediction(ticker, prediction, current_price, stock_df, combined_df, test_df, test_predictions, all_predictions, snapshot, sl_config=None):
    ...
    if sl_config is not None:
        ...
        with sc3:
            st.metric("ATR (14d)",
                      f"${latest_atr:.2f}" if latest_atr else "N/A")
        with sc4:
        ...
        if pred_triggered:
            ...
            # Override the signal displayed above
            st.markdown(
                "<div class='signal-badge signal-sell' style='font-size:14px;padding:6px 16px'>"
                "SELL (Stop Loss) 🛑</div>",
                unsafe_allow_html=True
            )
        else:
        ...
        # Add stop level to chart (done by patching fig before st.plotly_chart)
        # We store the stop price in session state for chart rendering
    ...
    if stop_px:
        ...
        # Mark historical bars where price was below stop
